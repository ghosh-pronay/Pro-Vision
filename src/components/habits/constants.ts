...
export interface Habit {
  ...
  checkInNotes?: { date: number; note: string }[];
  archived?: boolean;
  createdAt?: number;
  reminderTime?: string;
...
export const SORT_OPTIONS = [
  ...
  { value: "completion", label: { en: "Completion Rate", bn: "সম্পন্ন হার" } },
...
export function getHabitStreak(completedDates: number[]): number {
...
export function getCompletionRate(
  ...
  return completedDates.length
    ? Math.min(
        100,
        Math.round(
          (completedDates.filter((d: number) => d > todayStart - 30 * 86400000)
            .length /
            30) *
            100,
        ),
