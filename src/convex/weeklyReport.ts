...
export const sendWeeklyReport = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const user of users) {
      ...
      const completedTasks = tasks.filter((t: Doc<"tasks">) => t.completed).length;
      const totalTasks = tasks.length;

      const habitCheckins = habits.reduce(
        (sum: number, h: Doc<"habits">) =>
          sum +
          h.completedDates.filter((d) => d >= weekAgo && d <= now).length,
        0
      );

      const avgMood =
        moods.length > 0
          ? moods.reduce((sum: number, m: Doc<"moods">) => sum + m.value, 0) / moods.length
          : 0;
      ...
      await ctx.scheduler.runAfter(0, internal.weeklyReport.sendWeeklyReportEmail, {
        userId,
        email,
        report,
      });
...
export const sendWeeklyReportEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!res.ok) {
      ...
      console.error(`[weeklyReport] Resend API error (${res.status}): ${errText}`);
    }
  },
});
