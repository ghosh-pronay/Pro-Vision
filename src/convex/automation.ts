...
export const createRule = mutation({
  args: {
    ...
    trigger: v.union(v.literal("time"), v.literal("event"), v.literal("pattern")),
    triggerConfig: v.any(),
    action: v.union(
      v.literal("notify"),
      v.literal("create"),
...
export const generateSuggestions = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const transactions = await ctx.runQuery(internal.transactions.listByDateRange, {
      userId: args.userId,
      startDate: weekAgo,
      endDate: now,
    });

    const habits = await ctx.runQuery(internal.habits.listByUser, {
      userId: args.userId,
    });

    const tasks = await ctx.runQuery(internal.tasks.listByUser, {
      userId: args.userId,
    });
...
export const getSmartSuggestions = query({
  ...
  handler: async (ctx) => {
    ...
    const lowStreakHabits = habits.filter((h) => (h.streak || 0) < 3 && (h.streak || 0) > 0);
    if (lowStreakHabits.length > 0) {
      suggestions.push({
        type: "habits",
        title: "Habits Need Attention",
    ...
    const habitsWithoutStreak = habits.filter((h) => !h.streak || h.streak === 0);
