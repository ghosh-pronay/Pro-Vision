...
export const balanceSheet = query({
  ...
  handler: async (ctx, args) => {
    ...
    const cashAndEquivalents = wallets
      .filter((w) => w.type === "cash" || w.type === "bank" || w.type === "savings" || w.type === "digital")
    ...
    const loansGivenTotal = loans
      .filter((l) => l.type === "given" && l.status !== "paid" && l.createdAt <= asOf)
    ...
    const shortTermLoans = loans
      .filter((l) => l.type === "taken" && l.status !== "paid" && l.dueDate && l.dueDate <= shortTermCutoff && l.createdAt <= asOf)
      .reduce((sum, l) => sum + l.remainingAmount, 0);
    ...
    return {
      ...
      assets: {
        ...
        currentBreakdown: [
          { label: "Cash and Cash Equivalents", amount: cashAndEquivalents, breakdown: cashBreakdown },
  ...
  handler: async (ctx, args) => {
    ...
    for (const t of transactions) {
      ...
      if (t.date >= compStart && t.date < compEnd) {
        if (isIncome) {
          compIncomeCategories[t.category] = (compIncomeCategories[t.category] || 0) + t.amount;
          compIncome += t.amount;
        } else {
          compExpenseCategories[t.category] = (compExpenseCategories[t.category] || 0) + t.amount;
    ...
    const periodIncomeByCategory = Object.entries(incomeCategories)
      .map(([name, amount]) => ({ name, amount, pct: periodIncome > 0 ? Math.round((amount / periodIncome) * 100) : 0 }))
      .sort((a, b) => b.amount - a.amount);

    const periodExpenseByCategory = Object.entries(expenseCategories)
      .map(([name, amount]) => ({ name, amount, pct: periodExpense > 0 ? Math.round((amount / periodExpense) * 100) : 0 }))
      .sort((a, b) => b.amount - a.amount);
    ...
    const incomeChange = compIncome > 0
      ? Math.round(((periodIncome - compIncome) / compIncome) * 100)
      : periodIncome > 0 ? 100 : 0;

    const expenseChange = compExpense > 0
